...
import numpy as np
import sys
sys.path.append("../../../unitree_z1/z1_sdk/lib")
sys.path.append("../../../unitree_z1/z1_sdk/examples_py")
...
class Z1HALNode(Node):
    ...
    def get_arm_trajectory_generator_callback(self, msg):
        ...
        self.desired_arm_joints_position = np.array(msg.desired_arm_joints_position)
        self.desired_arm_joints_velocity = np.array(msg.desired_arm_joints_velocity)
        desired_arm_joints_torque = np.zeros(6)
        self.Kp_arm = np.array(msg.arm_kp)[0]
        self.Kd_arm = np.array(msg.arm_kd)[0]

        self.desired_gripper_position = msg.desired_gripper_position
        self.desired_gripper_velocity = 0.0
        desired_gripper_torque = 0.0

        self.arm.setArmCmd(self.desired_arm_joints_position, self.desired_arm_joints_velocity, desired_arm_joints_torque)
        self.arm.setGripperCmd(self.desired_gripper_position, self.desired_gripper_velocity, desired_gripper_torque)
        self.arm._ctrlComp.lowcmd.setControlGain(self.Kp_arm, self.Kd_arm)
