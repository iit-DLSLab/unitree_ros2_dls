...
class LowLevelCmdNode : public rclcpp::Node {
  public:
    explicit LowLevelCmdNode() : Node("low_level_cmd_node") {
      Init();
      Start();
      std::cout << "HAL A2 started correctly." << std::endl;
  }
  ...
  private:
    ...
    unitree_hg::msg::LowCmd low_cmd_;      // default init
    unitree_hg::msg::LowState low_state_;  // default init
    unitree_hg::msg::MotorState motor_[12];  // Unitree motor state message
...
void LowLevelCmdNode::Init() {
  ...
  InitLowCmd(); 
  low_cmd_pub_ = this->create_publisher<unitree_hg::msg::LowCmd>("/lowcmd", 1);
